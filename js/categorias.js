let catdatamvd = [{
    nombre:"Pitas Frias",
    descripcion:"Pan plano de la región mediterránea, con rellenos frios.",
    imagen:"img/spollo.jpg",
    lista:2
  },{
    nombre:"Pitas Calientes",
    descripcion:"Pan plano de la región mediterránea, con rellenos calientes.",
    imagen:"img/stibio.jpg",
    lista:1
  },{
    nombre:"Focaccias",
    descripcion:"Plato tradicional de la cocina italiana muy relacionado con la pizza. Utiliza el mismo pan pero lleva hierbas, aceite de oliva y parmesano encima.",
    imagen:"img/faa.jpg",
    lista:3
  },{
    nombre:"De Bar",
    descripcion:"Productos tipicos de bar rioplatense.",
    imagen:"img/faina.jpg",
    lista:4
  },{
    nombre:"Calzone",
    descripcion:"Similar a una empanada. Masa de pizza completamente cerrada y horneada.",
    imagen:"img/ctradicional.jpg",
    lista:5
  },{
    nombre:"Pizzas",
    descripcion:"Nuestra seleccion de pizzas gourmet",
    imagen:"img/muzza.jpg",
    lista:6
  },{
    nombre:"Dulces",
    descripcion:"Postres dulces de la casa",
    imagen:"img/cheesecake.jpg",
    lista:7
  },{
    nombre:"Bebidas",
    descripcion:"Cerveza, Refrescos de la linea Coca-Cola®",
    imagen:"img/wip.jpg",
    lista:8}];




    
    let pitasCalientesData = [{
        nombre:"Tradicional",
        descripcion:"Mozzarella, queso danbo y jamón",
        precio:260,
        imagen:"img/strad.jpg"
    },{
        nombre:"De la casa",
        descripcion:"Carne en tiritas, morrón, cebolla caramelizada y mozzarella",
        precio:345,
        imagen:"img/scasa.jpg"
    },{
        nombre:"Provoleta",
        descripcion:"queso provolone, finas hierbas",
        precio:280,
        imagen:"img/wip.jpg"
    },{
        nombre:"Vegetariana",
        descripcion:"Verduras salteadas, queso danbo, mozzarella",
        precio:345,
        imagen:"img/sveggie.jpg"
    },{
        nombre:"Pita 4 Quesos",
        descripcion:"Q. azul, Q. provolone, Q. mozzarella, Q. parmesano",
        precio:300,
        imagen:"img/wip.jpg"
    },{
        nombre:"Sandwich tibio de Jamón Crudo",
        descripcion:"Jamón crudo, Q. Brie, ciboulette, tomates, verdes y oliva en pan de Pita",
        precio:300,
        imagen:"img/stibio.jpg"
    }];
    
    let pitasFriasData = [{
        nombre:"Capresse",
        descripcion:"Mozzarella de bola, tomate en rodajas, albahaca, olivas",
        precio:260,
        imagen:"img/scapresse.jpg"
    },{
        nombre:"La fresca",
        descripcion:"Hojas de rúcula, bondiola, tomate, lascas de parmesano, oliva",
        precio:270,
        imagen:"img/wip.jpg"
    },{
        nombre:"Sandwich de Pollo",
        descripcion:"Verdes, pollo, palta, parmesano, aderezo",
        precio:345,
        imagen:"img/spollo.jpg"
    },{
        nombre:"Sandwich de roast beef",
        descripcion:"Verdes, roast beef asado, tomate, huevo, pepinillo, salsa tártara",
        precio:355,
        imagen:"img/sroast.jpg"
    }];
    
    
    
    
    
    
    
    
    let FocacciasData = [{
        nombre:"Focaccia de finas hierbas",
        descripcion:"Parmesano, oregano, romero, oliva y sal gruesa",
        precio:220,
        imagen:"img/ffh.jpg"
    },{
        nombre:"Focacccia de ajo asado y mozzarella",
        descripcion:"Ajos asados y pisados derretidos junto con mozzarella",
        precio:400,
        imagen:"img/faa.jpg"
    },{
        nombre:"Focaccia de verduras escalivadas",
        descripcion:"Focaccia, rúcula, verduras de estación asadas, tomate, queso de cabra, olivas",
        precio:480,
        imagen:"img/Escalivada.jpg"
    },{
        nombre:"Focaccia filipo",
        descripcion:"mezclum de hojas verdes, tomates frescos, mozzarella de búfala, tapenade, olivas",
        precio:440,
        imagen:"img/wip.jpg"
    }];
    
    
    
    
    
    
    
    
    
    
    
    
    
    let BarData = [{
        nombre:"Mozzarella",
        descripcion:"Clasica pizza de bar con Mozzarella(1/4 metro)",
        precio:220,
        imagen:"img/muzza.jpg"
    },{
        nombre:"Pizza",
        descripcion:"Clasica pizza de bar (1/4 metro)",
        precio:110,
        imagen:"img/wip.jpg"
    },{
        nombre:"Fainá",
        descripcion:"1 Porcion",
        precio:110,
        imagen:"img/faina.jpg"
    },{
        nombre:"Figazza",
        descripcion:"(1/4 metro)",
        precio:160,
        imagen:"img/figga.jpg"
    },{
        nombre:"Figazza con mozzarella",
        descripcion:"(1/4 metro)",
        precio:220,
        imagen:"img/wip.jpg"
    },{
        nombre:"Gustos extras",
        descripcion:"ananá, jamón, panceta, hongos, palmitos, pepperoni, aceitunas, roquefort",
        precio:40,
        imagen:"img/wip.jpg"
    },{
        nombre:"Extra muzza",
        descripcion:"",
        precio:50,
        imagen:"img/wip.jpg"
    }];






    let CalzoneData = [{
        nombre:"Tradicional",
        descripcion:"Jamón cocido, mozzarella, ricota",
        precio:500,
        imagen:"img/wip.jpg"
    },{
        nombre:"Capresse",
        descripcion:"Tomate fresco, albahaca, ricota italiana, mozzarella",
        precio:520,
        imagen:"img/ccapr.jpg"
    },{
        nombre:"Pepperoni",
        descripcion:"Mozzarella, pepperoni, tomate, orégano",
        precio:530,
        imagen:"img/wip.jpg"
    },{
        nombre:"Pollo y verduras",
        descripcion:"queso dambo, pollo salteado, verduras de estación, queso crema y ciboulette",
        precio:580,
        imagen:"img/wip.jpg"
    },{
        nombre:"Vegetales al wok",
        descripcion:"queso crema, mozzarella y vegetales al wok",
        precio:560,
        imagen:"img/wip.jpg"
    }];
    
    
    
    
    
    
    
    
    
    
    
    let PizzasData = [{
        nombre:"Pomodoro",
        descripcion:"Salsa de tomate",
        precio:325,
        imagen:"img/wip.jpg"
    },{
        nombre:"Margarita",
        descripcion:"S. de tomate, mozzarella, albahaca",
        precio:390,
        imagen:"img/marga.png"
    },{
        nombre:"Capresse",
        descripcion:"S. de tomate, mozzarella, tomate en rodajas, albahaca",
        precio:410,
        imagen:"img/wip.jpg"
    },{
        nombre:"Napoli",
        descripcion:"S. de tomate, mozzarella, tomate en rodajas, jamón, albahaca",
        precio:430,
        imagen:"img/wip.jpg"
    },{
        nombre:"Pepperoni",
        descripcion:"S. de tomate, mozzarella, pepperoni, orégano",
        precio:430,
        imagen:"img/wip.jpg"
    },{
        nombre:"Marinara",
        descripcion:"S. de tomate, anchoas, alcaparras, aceitunas negras, ciboullette",
        precio:390,
        imagen:"img/wip.jpg"
    },{
        nombre:"Capriccioso",
        descripcion:"S. de tomate, mozzarella, panceta crocante, hongos, ciboullette",
        precio:450,
        imagen:"img/wip.jpg"
    },{
        nombre:"Bella",
        descripcion:"S. de tomate, mozzarella, rúcula, pesto de tomates secos, lascas de parmesano",
        precio:460,
        imagen:"img/bella.jpg"
    },{
        nombre:"Bondiola",
        descripcion:"S. de tomate, mozzarella, bondiola, rúcula, tapenade",
        precio:460,
        imagen:"img/wip.jpg"
    },{
        nombre:"Pollo y verduras",
        descripcion:"S. de tomate, mozzarella, pollo y puerros salteados, salsa BBQ",
        precio:510,
        imagen:"img/wip.jpg"
    },{
        nombre:"Mexican",
        descripcion:"S. de tomate, mozzarella, carne en tiritas, morrón, cebollas salteadas",
        precio:510,
        imagen:"img/wip.jpg"
    },{
        nombre:"Hawaiana",
        descripcion:"S. de tomate, mozzarella, jamón, palmitos",
        precio:440,
        imagen:"img/wip.jpg"
    },{
        nombre:"Olivas",
        descripcion:"S. de tomate, mozzarella, aceitunas negras y verdes",
        precio:420,
        imagen:"img/wip.jpg"
    },{
        nombre:"Tropical",
        descripcion:"S. de tomate, mozzarella, ananá glaseado",
        precio:440,
        imagen:"img/olivas.jpg"
    },{
        nombre:"Fugaceta",
        descripcion:"Cebolla rehogada en oliva, provolone y crema",
        precio:460,
        imagen:"img/fugaceta.jpg"
    },{
        nombre:"Quattro Formagio",
        descripcion:"S. de tomate, mozzarella, provolone, parmesano, roquefort",
        precio:480,
        imagen:"img/wip.jpg"
    },{
        nombre:"Bianca",
        descripcion:"Queso crema, cebolla verdeo, panceta crocante, ciboulette",
        precio:480,
        imagen:"img/bianca.png"
    },{
        nombre:"Campesina",
        descripcion:"S. de tomate, mozzarella, rúcula, aceitunas negras, hongos, queso de cabra",
        precio:490,
        imagen:"img/wip.jpg"
    },{
        nombre:"Melanzane parmigiana",
        descripcion:"S. de tomate, berenjena grillada, mozzarella, parmesano",
        precio:470,
        imagen:"img/melanzane.jpg"
    },{
        nombre:"Prosciutto",
        descripcion:"s. de tomate, mozzarella, rúcula, jamón crudo, cherry, aceitunas negras",
        precio:530,
        imagen:"img/wip.jpg"
    }];
    
    
    





    let DulcesData = [{
        nombre:"Crocante de manzana",
        descripcion:"Crocante de manteca y azucar. Manzanas con canela junto con una bocha de helado",
        precio:180,
        imagen:"img/wip.jpg"
    },{
        nombre:"Cheescake de chocolate blanco",
        descripcion:"Galleta de chocolate, cheescake de chocolate blanco y salsa de frutos rojos.",
        precio:180,
        imagen:"img/wip.jpg"
    },{
        nombre:"Brownie de Chocolate",
        descripcion:"Brownie de chocolate, super cremoso con un una bocha de helado.",
        precio:180,
        imagen:"img/wip.jpg"
    }];
    
    










    let BebidasData = [{
        nombre:"Coca Cola 600ml",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Coca Cola Light 600ml",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Coca Cola 1.5L",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Coca Cola Light 1.5L",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Pilsen 1L",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Stella Artois 1L",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Stella Artois 300cc",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Zillertal 1L",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Volcanica 500cc",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Corona 300cc",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Corona 710cc",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    },{
        nombre:"Sin alcohol 330cc",
        descripcion:"",
        precio:"",
        imagen:"img/wip.jpg"
    }];
    




















































let catdatapde = [{
    nombre:"Pizzas",
    descripcion:"Nuestra seleccion de pizzas gourmet",
    imagen:"img/bianca.png",
    lista:9
    },{
    nombre:"Calzone",
    descripcion:"Similar a una empanada. Masa de pizza completamente cerrada y horneada.",
    imagen:"img/ccapr.jpg",
    lista:10
    },{
    nombre:"De Bar",
    descripcion:"Productos tipicos de bar rioplatense.",
    imagen:"img/faina.jpg",
    lista:11
    },{
    nombre:"Refuerzos",
    descripcion:"Refuerzos en pan focaccia, con los más frescos ingredientes",
    imagen:"img/stibio.jpg",
    lista:12
    },{
    nombre:"Dulces",
    descripcion:"Postres dulces de la casa",
    imagen:"img/cheesecake.jpg",
    lista:13
    },{
    nombre:"Bebidas",
    descripcion:"Cerveza, Refrescos de la linea Coca-Cola®",
    imagen:"img/wip.jpg",
    lista:14}];





let BarPDEData = [{
    nombre:"Fainá",
    descripcion:"1 Porcion",
    precio:120,
    imagen:"img/faina.jpg"
},{
    nombre:"Focaccia del Día",
    descripcion:`
    Dips para acompañar ($50):
        - Queso crema y ciboullete
        - Tapenade
        - Pesto de tomates secos
        - Pesto de albahaca
        
    
    `,
    precio:160,
    imagen:"img/ffh.jpg"
}];


let RefuerzosPDEData = [{
    nombre:"Clásico",
    descripcion:"queso dambo, jamón y muzzarella",
    precio:280,
    imagen:"img/wip.jpg"
},{
    nombre:"Capresse",
    descripcion:"tomate fresco, mozzarella de bola, albahaca, pimienta negra y aceite de oliva",
    precio:280,
    imagen:"img/scapresse.jpg"
},{
    nombre:"Jamón Crudo",
    descripcion:"jamón crudo, queso brie, rúcula y pesto de tomates secos",
    precio:370,
    imagen:"img/stibio.jpg"
}];

let DulcesPDEData = [{
    nombre:"Pie de limón",
    descripcion:"",
    precio:200,
    imagen:"img/wip.jpg"
},{
    nombre:"Cheesecake ",
    descripcion:"con salsa de frutos rojos",
    precio:210,
    imagen:"img/cheesecake.jpg"
},{
    nombre:"Pastel de peras y frutos rojos",
    descripcion:"",
    precio:210,
    imagen:"img/wip.jpg"
}];





let CalzonePDEData = [{
    nombre:"De Bondiola",
    descripcion:"tomates asados, ricota cremosa, bondiola y mozzarella",
    precio:590,
    imagen:"img/ccapr.jpg"
},{
    nombre:"Pesca Fresca",
    descripcion:"pesca del día al horno con vegetales y queso fundido",
    precio:620,
    imagen:"img/wip.jpg"
},{
    nombre:"De Vegetales",
    descripcion:"verduras asadas, ricota cremosa y queso fundido",
    precio:610,
    imagen:"img/scapresse.jpg"
}];


let PizzasPDEData = [{
    nombre:"Margarita",
    descripcion:"salsa de tomate, mozzarella, albahaca y aceite de oliva",
    precio:420,
    imagen:"img/marga.png"
},{
    nombre:"Marinara",
    descripcion:"salsa de tomate, anchoas, ajos asados y toque de tapenade",
    precio:420,
    imagen:"img/wip.jpg"
},{
    nombre:"Escalivada",
    descripcion:"vegetales asados macerados en aceite especiado y rúcula",
    precio:520,
    imagen:"img/Escalivada.jpg"
},{
    nombre:"Jamón Crudo",
    descripcion:"salsa de tomate, mozzarella, jamón crudo, rúcula, parmesano y aceite de oliva",
    precio:570,
    imagen:"img/JC_1_.jpg"
},{
    nombre:"Bianca",
    descripcion:"queso crema, cebolla de verdeo, panceta crujiente y ciboulette",
    precio:525,
    imagen:"img/bianca.png"
},{
    nombre:"De Hongos",
    descripcion:"Champiñones frescos, cebollas asadas, queso provolone, toque de crema y pesto de albahaca",
    precio:490,
    imagen:"img/Hongos.jpg"
},{
    nombre:"Camarón",
    descripcion:"salsa de tomate, mozzarella y camarones al ajillo",
    precio:570,
    imagen:"img/Camaron.jpg"
},{
    nombre:"Cuatro quesos",
    descripcion:"salsa de tomate, provolone, mozzarella, parmesano y queso azu",
    precio:525,
    imagen:"img/wip.jpg"
}];







let BebidasPDEData = [{
    nombre:"Refresco 600ml",
    descripcion:"",
    precio:110,
    imagen:"img/wip.jpg"
},{
    nombre:"Aquarius",
    descripcion:"",
    precio:90,
    imagen:"img/wip.jpg"
},{
    nombre:"Refresco 1.5L",
    descripcion:"",
    precio:160,
    imagen:"img/wip.jpg"
},{
    nombre:"Cervezas 330cc",
    descripcion:"",
    precio:120,
    imagen:"img/wip.jpg"
},{
    nombre:"Cervezas 1L",
    descripcion:"",
    precio:250,
    imagen:"img/wip.jpg"
},{
    nombre:"Artesanales 500cc",
    descripcion:"",
    precio:220,
    imagen:"img/wip.jpg"
}];